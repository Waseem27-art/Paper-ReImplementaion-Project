...
def scale_data(conf, df_orig):
    # Ensure 'weights' is retrieved from the configuration
    weights = conf['Weights']  # Fetch weights from the configuration

    # Print the DataFrame description before scaling
    print('Before')
    print(df_orig.describe(include='all'))
    
    # Apply the weights to each column
    for col, weight in zip(list(df_orig.columns), weights):
        df_orig[col] = df_orig[col].apply(lambda x: x * weight)
        
    # Update the bounds based on weights if necessary
    bounds = conf['Bounds']  # Assuming 'Bounds' is present in the config
    bounds = [[bounds[i][x] * weight for x, weight in enumerate(weights)] for i in range(len(bounds))]
    
    # Print the DataFrame description after scaling
    print('After')
    print(df_orig.describe(include='all'))
    
    return df_orig, bounds
